...
export class ClientService {
    ...
    findByState(state: string): Observable<Client[]> {
        return this.http.get<Client[]>(`${this.apiUrl}/state/${state}`).pipe(
            catchError(this.handleError<Client[]>('findByState', []))
        );
    }
    ...
    delete(id: number): Observable<boolean> {
        return this.http.put(`${this.apiUrl}/delete/${id}`, null).pipe(
            tap(() => {
    ...
    restore(id: number): Observable<boolean> {
        return this.http.put(`${this.apiUrl}/restore/${id}`, null).pipe(
